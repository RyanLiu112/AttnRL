...
class RewardMathFnV2:
    ...
    def __call__(self, task_info: dict, action: str) -> RewardOutput:
        ...
        if self.config.use_math_verify:
            try:
                model_answer = math_verify.parse(model_solution, parsing_timeout=self.config.reward_timeout)
            except Exception:
            ...
            if model_answer[1] in processed_ground_truths:
            ...
            for ground_truth in processed_ground_truths:
                try:
                    if math_verify.verify(
                        math_verify.parse(f"\\boxed{{{ground_truth}}}", parsing_timeout=self.config.reward_timeout),
                        model_answer,
                        timeout_seconds=self.config.reward_timeout,
                    ):
                        return RewardOutput(reward=self.config.correct_reward, is_correct=True, pred=model_answer[1])
                except Exception:
                    continue
